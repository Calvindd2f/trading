...
def calculate_max_drawdown(equity):
    ...
    for value in equity:
        if value is None:
            logging.error(
                f"Error while calculating max drawdown: found null value in equity curve.")
            return max_drawdown
        if value > peak:
            peak = value
        else:
...
def calculate_macd(series, slow=26, fast=12, signal=9):
    ...
    for i in range(1, len(series)):
        exp1[i] = exp1[i-1] * (1 - 2 / (fast + 1)) + \
            series[i] * (2 / (fast + 1))
        exp2[i] = exp2[i-1] * (1 - 2 / (slow + 1)) + \
            series[i] * (2 / (slow + 1))
        macd[i] = exp1[i] - exp2[i]
        signal_line[i] = signal_line[i-1] * \
            (1 - 2 / (signal + 1)) + macd[i] * (2 / (signal + 1))
...
def execute_trade(side, amount):
    ...
    logging.info(f"Executed {side} trade for {trade_amount} at price {
                 trade_price}, stop loss at {stop_loss_price}")
